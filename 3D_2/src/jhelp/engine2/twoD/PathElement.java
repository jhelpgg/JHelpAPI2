...
public class PathElement
{
     ...
     * @param points   Points for the path element
     */
    public PathElement(final @NotNull PathType pathType, final @NotNull Point3D... points)
    {
    ...
    public void loadFromXML(final @NotNull MarkupXML markupXML) throws Exception
    {
        ...
        if (pathType.length() < 1)
        {
            throw new IllegalArgumentException(
                    UtilText.concatenate("Missing mandatory parameter ", ConstantsXML.MARKUP_PATH_ELEMENT_pathType,
                                         " in ",
                                         markupXML.getName()));
        }
        ...
        {
            this.points[i] = new Point3D(Float.parseFloat(stringTokenizer.nextToken()),
    ...
    public @NotNull MarkupXML saveToXML()
    {
        ...
        {
            stringBuilder.append(point3D.x());
            stringBuilder.append(' ');
            stringBuilder.append(point3D.y());
            stringBuilder.append(' ');
            stringBuilder.append(point3D.z());
            stringBuilder.append(' ');
        }
        markupXML.setText(stringBuilder.toString());
        return markupXML;
    }
}