...
import jhelp.engine2.twoD.BorderIterator;
import jhelp.util.debug.Debug;
import jhelp.util.text.UtilText;

/**
 * 3D font. It creates 3D object for character or String <br>
 * Warning: It not works for now !<br>
 * TODO Implements a tesslator for triangularize any polygon
 *
 * @author JHelp
 */
public class Font3D
{
     ...
     * @param character  Character to represents
     ...
     * @param multBorder U multiplier
     * @return Created object
     */
    public static NodeWithMaterial createCharacter(
            final String fontFamily, final char character, final float flatness, final float multBorder)
    {
        if (fontFamily == null)
        {
        ...
        final BorderIterator borderIterator = new BorderIterator(
                font.createGlyphVector(Font3D.fontRenderContext, new char[]
                        {
                                character
                        }).getOutline(), flatness);
        ...
        final float minX = (float) borderIterator.minimumX();
        final float minY = (float) borderIterator.minimumY();

        final float width  = (float) Math.abs(borderIterator.minimumX() - borderIterator.maximumX());
        final float height = (float) Math.abs(borderIterator.minimumY() - borderIterator.maximumY());

        final float tx = (float) ((borderIterator.minimumX() - borderIterator.maximumX()) / 2d);
        final float ty = (float) ((borderIterator.maximumY() - borderIterator.minimumY()) / 2d);

        final float length = (float) borderIterator.length();
        float       u      = 0;
        ...
        // TODO Compute the polygons
        Debug.todo("Compute the polygons");
      /*
      dx = dy = cx = cy = fx = fy = 0;
     ...
     *
     * @param fontFamily Font family name
     * @param string     String to represents
     * @param flatness   The maximum distance that the line segments used to approximate the curved segments are allowed to deviate from
     *                   any point on the original curve
     * @param multBorder U multiplier
     * @param space      Space between each letters
     * @return Created object
     */
    public static Node createString(
            final String fontFamily, final String string, final float flatness, final float multBorder,
            final float space)
    {
        ...
        for (int i = 0; i < length; i++)
        {
            ...
            virtualBox.add(box.minX() + x, box.minY(), box.minZ());
            virtualBox.add(box.maxX() + x, box.maxY(), box.maxZ());

            x += Math.abs(box.maxX() - box.minX()) + space;
        }

        final Point3D center = virtualBox.center();
        x = virtualBox.minX() - center.x;

        final Node node = new Node();
        node.nodeName = UtilText.concatenate("Auto_Generate_String_", string,
                                             "_Font_", fontFamily, '_', flatness, '_', multBorder, '_', space);
