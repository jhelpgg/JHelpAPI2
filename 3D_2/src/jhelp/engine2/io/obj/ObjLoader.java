...
public class ObjLoader
{
     ...
     *
     * @param object3D    Object to fill, if {@code null} a new instance is created
     * @param inputStream Stream to parse
     * @return Object created
     * @throws IOException On reading issue
     */
    public static Object3D loadObj(final @Nullable Object3D object3D, final @NotNull InputStream inputStream)
            throws IOException
    {
     ...
     *
     * @param object3D       Object to reuse. Can be {@code null} to create a new object
     * @param inputStream    Stream to read. The stream is not closed, it up to caller to do it
     * @param reverseNormals Indicates if normal have to be reversed
     * @return Created/modified object
     * @throws IOException On stream read issue
     */
    public static Object3D loadObj(
            @Nullable Object3D object3D, final @NotNull InputStream inputStream, final boolean reverseNormals)
            throws IOException
    {
        if (object3D == null)
        {
        ...
        final ArrayList<Point3D> normals = new ArrayList<>();
        final ArrayList<Point2D> uv      = new ArrayList<>();
        ...
        while (line != null)
        {
            ...
            if ((line.length() > 0) && (line.charAt(0) != '#'))
            {
                ...
                if (word.equals("g") || word.equals("o"))
                {
                    ...
                    {
                        Mesh.fillObjectOBJ(edit,
                                           points, normals, uv,
                                           pointFace, normalFace, uvFace,
                                           startPoint, startUV, startNormal);
                    ...
                    Debug.verbose("Found object : ", edit.nodeName);
                }
                else if (word.equals("v"))
                {
                    points.add(new Point3D(Float.parseFloat(stringTokenizer.nextToken()),
                                           Float.parseFloat(stringTokenizer.nextToken()),
                                           Float.parseFloat(stringTokenizer.nextToken())));
                }
                else if (word.equals("vt"))
                {
                    uv.add(new Point2D(Float.parseFloat(stringTokenizer.nextToken()),
                                       1 - Float.parseFloat(stringTokenizer.nextToken())));
                }
                else if (word.equals("vn"))
                {
                    normals.add(new Point3D(multiplier * Float.parseFloat(stringTokenizer.nextToken()),
