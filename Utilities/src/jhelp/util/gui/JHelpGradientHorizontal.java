        ...
        implements JHelpPaint
{
     ...
     *
     * @param colorStart Color at start (left)
     * @param colorEnd   Color at end (right)
     */
     ...
     * Add a color step
     *
     * @param percent Percent of the step
    ...
    public int obtainColor(final int x, final int y)
    {
        ...
        final Couple interval = this.percents.intervalOf(per);
        ...
        if (start < 0)
        {
            return this.percents.get(0).color;
        }

        if (end < 0)
        {
            return this.percents.get(start - 1).color;
        }

        if (start == end)
        {
            return this.percents.get(start).color;
        }

        final int col1 = this.percents.get(start).color;
        final int col2 = this.percents.get(end).color;

        start = this.percents.get(start).percent;
        end = this.percents.get(end).percent;
