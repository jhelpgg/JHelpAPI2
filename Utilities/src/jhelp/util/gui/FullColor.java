...
public class FullColor
{
    ...
    public static double computeHue(final int argb)
    {
        ...
        final double min    = Math2.minimum(r, g, b);
        final double max    = Math2.maximum(r, g, b);
        final double chroma = max - min;

        if (Math2.isNul(chroma))
        {
            return 0;
        }

        if (Math2.equals(max, r))
        {
            return 60.0 * Math2.modulo((g - b) / chroma, 6.0);
        }

        if (Math2.equals(max, g))
        {
     ...
     *
     * @param red   Percent of red (in [0-1])
     * @param green Percent of green (in [0-1])
     * @param blue  Percent of blue (in [0-1])
     */
    ...
    public FullColor(final int argb)
    {
        ...
        if (Math2.isNul(this.chroma))
        {
            this.hue = 0;
        }
        else if (Math2.equals(max, r))
        {
            this.hue = 60.0 * Math2.modulo((g - b) / this.chroma, 6.0);
        }
        else if (Math2.equals(max, g))
        {
        ...
        this.keyName = pair.first;
        this.keyDarkLight = pair.second;
    }
