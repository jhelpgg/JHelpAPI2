...
public final class UtilGUI
{
    ...
    public static KeyStroke createKeyStroke(
            final char character, final boolean control, final boolean alt,
    ...
    public static Rectangle getScreenBounds(final int screen)
    {
        ...
        final Insets insets = UtilGUI.TOOLKIT.getScreenInsets(
                UtilGUI.GRAPHICS_DEVICES[screen].getDefaultConfiguration());
    ...
    public static int obtainScreenIndex(final Window window)
    {
        ...
        int                   areaMax               = Math2.computeIntersectedArea(windowBounds, bounds);
        int                   screenIndex           = 0;
        int                   area;
        for (int i = 1; i < UtilGUI.GRAPHICS_DEVICES.length; i++)
        {
    ...
    public static BufferedImage screenShot()
    {
        ...
        int width  = Math.max(xMax - xMin, size.width);
        int height = Math.max(yMax - yMin, size.height);
