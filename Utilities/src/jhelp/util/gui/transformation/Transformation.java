...
public class Transformation
{
    ...
    private void check(final int x, final int y)
    {
        if ((x < 0) || (x >= this.width) || (y < 0) || (y >= this.height))
        {
            throw new IllegalArgumentException(
                    UtilText.concatenate('(', x, ", ", y, ") is outside the transformation ", this.width, 'x',
                                         this.height));
        }
    }
     ...
     * @param x    Pixel X
     * @param y    Pixel Y
     * @param vect Vector transformation
     */
    ...
    public void toHorizontalEllpticArc(final double factor)
    {
        ...
        for (int x = 0; x < this.width; x++)
        {
            vy = (int) ((factor * Math.sqrt(ray - Math2.square(x - center))) - max);
            pos = x;
