        ...
        implements DynamicAnimation
{
    ...
    public final void addFrame(final int frame, final VALUE value)
    {
        ...
        synchronized (this.keyFrames)
        {
            ...
            {
                this.keyFrames.get(index).value = value;
            }
        }
    }
    ...
    public final boolean animate(final float absoluteFrame, final JHelpImage image)
    {
        synchronized (this.keyFrames)
        {
            ...
            KeyFrame<VALUE> keyframeFirst = this.keyFrames.get(0);
            KeyFrame<VALUE> keyframeLast  = this.keyFrames.get(size - 1);
            ...
            frame = 0;
            while ((frame < size) && (this.keyFrames.get(frame).frame < actualFrame))
            {
                frame++;
            }
            ...
            keyframeFirst = this.keyFrames.get(frame - 1);
            keyframeLast = this.keyFrames.get(frame);
