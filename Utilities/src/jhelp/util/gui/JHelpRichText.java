...
public final class JHelpRichText
{
            ...
            extends CacheElement<JHelpImage>
    {
         ...
         *
         * @param resources Resources set where found the image
         * @param resource  Image resource name
         * @param size      Image desired size
         */
        ...
        protected JHelpImage create()
        {
            try
            {
                return JHelpImage.loadImageThumb(this.resources.obtainResourceStream(this.resource), this.size,
                                                 this.size);
            }
            catch (final IOException exception)
            {
                Debug.exception(exception, "Failed to load resource ", this.resource);
     ...
     * Images cache
     */
    private final Cache<JHelpImage> CACHE_IMAGES = new Cache<>();
    /**
     * Resources set
     */
    private final Resources           resources;
     ...
     *
     * @param resources Resources set where find images
     */
    public JHelpRichText(final Resources resources)
    {
        ...
        this.resources = resources;
        this.symbols = new SortedArray<>(Symbol.class, true);
    }
     ...
     * @param colorText   Color for text (Used if paintText and texturePaint are {@code null})
     * @param paintText   Paint for text (Used if not {@code null} and texturePaint is {@code null})
     * @param textureText Texture for text (Used if not {@code null})
     * @return Created image
     */
    private JHelpImage createImage(
            final String text, final JHelpFont font, final int colorText,
        ...
        while (line != null)
        {
            ...
            for (int i = 0; i < size; i += 2)
            {
                ...
                if (index < start)
                {
                    ...
                    elements.add(new Triplet<>(true, new Point(x, y), part));
                ...
                elements.add(new Triplet<>(false, new Point(x, y), this.symbols.get(sy)
                                                                               .getResource()));
     ...
     *
     * @param symbol   Symbol to associate
     * @param resource Image resource name
     */
     ...
     * @param text        Text to convert
     * @param font        Font to use for text
     * @param textureText Text texture
     * @return Created image
     */
