        ...
        implements Comparable<MayaNumber>
{
    static
    {
        ...
        {
            Debug.exception(exception, "Failed to get Maya font");
        }
        ...
        try
        {
            font = JHelpFont.createFont(Type.TRUE_TYPE,
                                        MayaNumber.class.getResourceAsStream("Roboto-Regular-Numbers.ttf"), 32,
                                        Value.FREE, Value.FREE, false);
        }
        catch (final Exception exception)
        {
            Debug.exception(exception, "Failed to get Number font");
        }
     ...
     * String MUST only contains defined digits and mustn't be {@code null} or empty
     *
    ...
    {
        return Math2.sign(this.number - mayaNumber.number);
    }
        ...
        for (int i = 0; i < length; i++)
        {
            try
            {
                image = JHelpImage.loadImageThumb(
                        MayaNumber.class.getResourceAsStream(MayaNumber.TOTEM_NUMBERS[this.base20[i] & 0xFF]), baseSize,
                        baseSize);
                totem.drawImage(x, y, image, false);
            }
            catch (final Exception exception)
            {
                Debug.exception(exception, "Failed to get image for number ", this.base20[i] & 0xFF);
            }
