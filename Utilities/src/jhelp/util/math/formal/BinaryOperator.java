        ...
        extends Function
{
    ...
    protected static Function parseBinaryOperator(final String string)
    {
            ...
            for (int i = 0; (i < nbOfCharacter) && (operatorIndex < 0); i++)
            {
                ...
                switch (car)
                {
                    ...
                    case ')':
                        ...
                        break;
                    default:
                        if ((parenthesisDeepCount == 0) &&
                            (Utilities.indexOf(BinaryOperator.OPERATORS, String.valueOf(car)) >= 0))
                        {
                            operatorIndex = i;
                        }
                        break;
        ...
        if (index >= 0)
        {
            ...
            switch (index)
            {
                case BinaryOperator.INDEX_POWER:
                    if (func2 instanceof Constant)
                    {
                        ...
                        if (constant.isPositive() == true)
                        {
                            ...
                            if ((integer < 100) && (Math2.equals(value, integer) == true))
                            {
                ...
                case BinaryOperator.INDEX_ADDITION: // Addition
                    return new Addition(func1, func2);
                case BinaryOperator.INDEX_SUBTRACTION: // Substraction
                    return new Subtraction(func1, func2);
                case BinaryOperator.INDEX_MULTIPLICATION: // Multiplication
                    return new Multiplication(func1, func2);
                case BinaryOperator.INDEX_DIVIDE: // Division
    ...
    public BinaryOperator(
            final String operatorStringRepresentation, final Function parameter1, final Function parameter2)
    {
    ...
    public String toString()
    {
        ...
        if (!(this.parameter1 instanceof Constant) && !(this.parameter1 instanceof Variable) &&
            !(this.parameter1 instanceof UnaryOperator))
        {
            string1.insert(0, '(');
            string1.append(')');
        }
        ...
        if (!(this.parameter2 instanceof Constant) && !(this.parameter2 instanceof Variable) &&
            !(this.parameter2 instanceof UnaryOperator))
        {
            string2.insert(0, '(');
            string2.append(')');
        }
