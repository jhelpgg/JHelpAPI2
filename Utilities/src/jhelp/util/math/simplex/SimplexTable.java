...
public class SimplexTable
{
     ...
     * @param variables    Variables/parameters name
     * @param slackSurplus Slack/surplus name
     * @param values       Table values
     * @param answers      Table answers
     */
    public SimplexTable(
            final char objective, final char[] variables, final char[] slackSurplus, final int[][] values,
            final int[] answers)
    {
        // Basics check
        ...
        for (int i = values.length - 1; i >= 0; i--)
        {
            if (values[i].length != lineLength)
            {
                throw new IllegalArgumentException(
                        "The line " + i +
                        " of values MUST have length equal to number of variable + number of slack/surplus +1(for objective)");
            }
        }
    ...
    private int columnForEliminate()
    {
        ...
        for (int i = 0; i < length; i++)
        {
            ...
            if (this.table[column + (i * this.width)] < 0)
            {
                ...
                if (column < 0)
                {
                    Debug.warning("Column for eliminate not found since their a negative basics : ",
                                  this.basics[i]);
                }
    ...
    private void doPivot(final int pivot)
    {
        ...
        final int signPivot  = Math2.sign(valuePivot);
        ...
        for (int i = this.basics.length; i >= 0; i--)
        {
            if (i != line)
            {
                ...
                if (valueOther != 0)
                {
                    ...
                    gcd = Math2.greaterCommonDivider(coefOther, coefPivot);
                    coefOther /= gcd;
                    coefPivot /= gcd;
                    ...
                    {
                        this.table[indexOther] =
                                (coefOther * this.table[indexOther]) + (coefPivot * this.table[indexPivot]);
                    }
                }
            }
    ...
    private int pivotInColumn(final int column)
    {
        ...
        if (pivot < 0)
        {
            Debug.warning("pivot not found, may be their an issue ! Or for the current table may be have no solution");
        }
    ...
    public SimplexResult resolveTable()
    {
        ...
        for (int i = this.basics.length - 1; i >= 0; i--)
        {
            ...
            result[index] =
                    (double) this.table[((i + 1) * this.width) - 1] / (double) this.table[index + (i * this.width)];
        }

        result[this.parameters.length] =
                (double) this.table[this.table.length - 1] / (double) this.table[this.table.length - 2];
