 ...
 *
 * @param <BINARIZABLE> Type to read
 * @author JHelp
 */
public class PipeReaderManager<BINARIZABLE extends Binarizable>
{
     ...
     */
    class TaskReadPipe
            implements ConsumerTask<PipeReader>
    {
        /**
         ...
         * {@inheritDoc}
         *
         * @param parameter Pipe reader to use
         * @see jhelp.util.thread.ConsumerTask#consume(Object)
         */
        @Override
        public void consume(final PipeReader parameter)
        {
            try
            {
            ...
            synchronized (PipeReaderManager.this.alive)
            {
                ...
                {
                    ThreadManager.parallel(this, parameter);
                }
            }
        }
    }
    ...
    class TaskSignalMessage<B extends Binarizable>
            implements ConsumerTask<Triplet<PipeReaderManagerListener<B>, PipeReader, B>>
    {
        /**
         ...
         * {@inheritDoc}
         *
         * @param parameter Triplet of listener to alert, used piper reader and the message itself
         * @see jhelp.util.thread.ConsumerTask#consume(Object)
         */
        @Override
        public void consume(final Triplet<PipeReaderManagerListener<B>, PipeReader, B> parameter)
        {
     ...
     *
     * @param binarizebleClass Type of binarizable to read
     * @param pipeReader       Pipe reader to use
     */
    public PipeReaderManager(final Class<BINARIZABLE> binarizebleClass, final PipeReader pipeReader)
    {
        if (binarizebleClass == null)
        {
            throw new NullPointerException("binarizebleClass mustn't be null");
        }

        if (pipeReader == null)
        {
            throw new NullPointerException("pipeReader mustn't be null");
        }
    ...
    void signalToListenersMessage(final BINARIZABLE message)
    {
        synchronized (this.listeners)
        {
            ...
            {
                ThreadManager.parallel(this.taskSignalListener,
                                       new Triplet<PipeReaderManagerListener<BINARIZABLE>, PipeReader, BINARIZABLE>(
    ...
    public void startManagement()
    {
        ...
        ThreadManager.parallel(this.taskReadPipe, this.pipeReader);
    }
