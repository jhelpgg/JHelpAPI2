        ...
        extends AnimationKeyFrame<Node, PositionNode>
{
    ...
    protected void interpolateValue(
            final Node object, final PositionNode before, final PositionNode after, final float percent)
    {
        float anti;

        anti = 1f - percent;

        object.setPosition((before.x * anti) + (after.x * percent), (before.y * anti) + (after.y * percent),
