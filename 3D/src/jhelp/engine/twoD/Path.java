...
public class Path
{
    ...
    {
        this.path.add(
                new PathElement(PathType.CUBIC, new Point3D(startPoint, start), new Point3D(controlPoint1, control1),
    ...
    public float computePathSize()
    {
        ...
        for (final PathElement pathElement : this.path)
        {
            ...
            {
                this.size += Math.sqrt(Math3D.square(points[i - 1].getX() - points[i].getX()) +
                                       Math3D.square(points[i - 1].getY() - points[i].getY()));
            }
            old = points[length - 1];
        }
