        ...
        extends JHelpComponent2D
{
     ...
     * @param componentView3D Component that carry the 3D
     * @param frame           Frame where lies the component
     */
    JHelpComponent2DView3D(
            final int width, final int height, final ComponentView3D componentView3D, final JHelp2DFrame frame)
    {
    ...
    protected void paint(final int x, final int y, final JHelpImage parent)
    {
        ...
        if (((this.previousX != x) || (this.previousY != y) || (this.previousWidth != rectangle.width) ||
        ...
        if (this.alreadyAdded == false)
        {
            ...
            {
                Debug.warning("Impossible to start the renderer !");
            }
        }
    }
    ...
    {
        final JHelpMouseListener listener = this.mouseListener();
     ...
     *
     * @param clickInSpaceListener The click in empty space listener (Can us {@code null} for remove the current listener)
     */
     ...
     *
     * @param keyListener New key listener or {@code null} to remove key listener
     */
