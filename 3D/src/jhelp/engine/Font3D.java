...
public class Font3D
{
        ...
        while (borderIterator.isDone() == false)
        {
            switch (borderIterator.currentSegment(temp))
            {
                case PathIterator.SEG_MOVETO:
                    ...
                    polygon3D.push(
                            new Vertex(tx + cx, ty + cy, z, (cx + minX) / width, (cy + minY) / height, 0, 0, -1));
                    break;
                case PathIterator.SEG_LINETO:
                    fx = temp[0];
                    fy = temp[1];
                    ...
                    polygon3D.push(
                            new Vertex(tx + fx, ty + fy, z, (fx + minX) / width, (fy + minY) / height, 0, 0, -1));
                ...
                case PathIterator.SEG_CLOSE:
                    polygon3D.push(
                            new Vertex(tx + dx, ty + dy, z, (dx + minX) / width, (dy + minY) / height, 0, 0, -1));
            ...
            final float space)
    {
        ...
        node.nodeName = ((new StringBuffer("Auto_Generate_String_").append(string)
                                                                   .append("_Font_")
                                                                   .append(fontFamily)
                                                                   .append('_')
                                                                   .append(flatness)
