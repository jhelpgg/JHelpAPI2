...
public class Tool3D
{
     ...
     *
     * @param helpSceneRenderer Renderer to explore
     ...
     */
    public static Mesh createJoinedMesh(
            final Path pathU, final int precisionU, final Path pathV, final int precisionV, final float multU,
            ...
            for (final Line2D lineV : linesV)
            {
                ...
                if (first == false)
                {
                        ...
                        if (Math3D.nul(length) == false)
                        {
                            ...
                            if (Math3D.equal(Math.abs((vx00 * vx10) + (vy00 * vy10) + (vz00 * vz10)), 1f) == false)
                            {
                                ...
                                if (linearize == true)
                                {
                                    ...
                                    u00 += (float) Math.sqrt(Math3D.square(mx0 - p0.getPosition().getX()) +
                                                             Math3D.square(my0 - p0.getPosition().getY())
                                                             + Math3D.square(mz0 - p0.getPosition().getZ()));
                                    u01 += (float) Math.sqrt(Math3D.square(mx1 - p1.getPosition().getX()) +
                                                             Math3D.square(my1 - p1.getPosition().getY())
                    ...
                    if (linearize == true)
                    {
                        ...
                        u00 += (float) Math.sqrt(Math3D.square(x00 - p0.getPosition().getX()) +
                                                 Math3D.square(y00 - p0.getPosition().getY())
                                                 + Math3D.square(z00 - p0.getPosition().getZ()));
                        u01 += (float) Math.sqrt(Math3D.square(x01 - p1.getPosition().getX()) +
                                                 Math3D.square(y01 - p1.getPosition().getY())
    ...
    public static float futureIntersectionVolume(
            final Node node1, final Point3D vector1, final Node node2, final Point3D vector2)
    {
     ...
     * @return Bump texture
     */
    public static Texture obtainBumpTexture(
            final Color color, final Texture bump, final float contrast, final float dark, final int shiftX,
            final int shiftY)
    {
        return Tool3D.obtainBumpTexture(new Texture("Unify", bump.getWidth(), bump.getHeight(), color), bump, contrast,
                                        dark, shiftX, shiftY);
    }
     ...
     */
    public static Texture obtainBumpTexture(
            final Texture original, final Texture bump, float contrast, final float dark, final int shiftX,
            final int shiftY)
    {
