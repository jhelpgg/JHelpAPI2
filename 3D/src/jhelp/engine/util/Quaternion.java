...
public class Quaternion
{
     ...
     */
    public static Quaternion computeInterpolation(
            final Quaternion quaternionStart, final Quaternion quaternionEnd, final float percent,
            Quaternion quaternionInterpolate)
    {
        if (quaternionStart == null)
        {
            throw new NullPointerException("The quaternionStart mustn't be null !");
        }

        if (quaternionEnd == null)
        {
            throw new NullPointerException("The quaternionEnd mustn't be null !");
        }
        ...
        float dot = (startX * quaternionEnd.x) + (startY * quaternionEnd.y) + (startZ * quaternionEnd.z) +
                    (startW * quaternionEnd.w);
        if (dot < 0)
        {
            startX *= -1;
            startY *= -1;
    ...
    public static Quaternion createQuaternion(
            final float rotationX, final float rotationY, final float rotationZ, final float rotationAngle)
    {
        final Quaternion quaternion = new Quaternion();
        quaternion.w = (float) Math.cos(rotationAngle / 2f);
        final float size = (float) Math.sqrt(
    ...
    public Quaternion multiply(final Quaternion quaternion)
    {
        return new Quaternion(//
                              ...
                              (this.z * quaternion.y), //
                              ((this.w * quaternion.y) + (this.y * quaternion.w) + (this.x * quaternion.z)) -
                              (this.z * quaternion.x), //
